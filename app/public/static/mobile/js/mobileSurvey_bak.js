...
function errorfunc(data){
	...
	mobilealertokMsg("error!errorcode:"+errorcode+","+errormsg,"确定");
}
...
function setSurvey(list,pagingType,pagenumnow){
	...
	if(pagingType==1){
		...
		"<li data-page='1'>首页</li>";
		if(pagenumnow>1){
			pagingHtml += "<li data-page='"+(pagenumnow-1)+"'>上一页</li>";
		}else{
			pagingHtml += "<li data-page='1' class='disable'>上一页</li>";
		}
		if(pagelength>=3){
		...
		if(pagelength==pagenumnow){
			pagingHtml+="<li data-page='' class='disable'>下一页</li>";
		}else{
			pagingHtml+="<li data-page='"+(pagenumnow+1)+"'>下一页</li>";
		}
		pagingHtml+="<li data-page='"+pagelength+"'>末页</li>"+
		"</ul>";
		...
		//单选题和多选题，选项一样的
			...
			if(type==2){
				...
				$thisquestion.on("click",".content .option",function(){
					...
					var min = min_input;
					setTimeout(function(){
						var selectedOptions = $this.parent().find(".ui.checkbox input:checked").length;	
...
//						}
		...
		//量表题
			...
			//组装第一行标题
			//TODO test firstTr is <tr>element?
...
function sendAnswer(){
	...
	function func(data){
//		mobilealertokMsg("答卷成功！","确定",function(){window.location.href="/views/mobile/mobileSurveyList.jsp";});
...
function getAnswerJson(){
	...
	for(var q=0;q<$(".question").length;q++){
		...
		if($thisquestion.hasClass("dx")){//单选
			//校验必答
			if(required && $thisquestion.find(".content .option .ui.checkbox.checked").length==0){
				$(".error h4").html("题目"+questionno+"为必答题！");
				return false;
			}else if(!required && $thisquestion.find(".content .option .ui.checkbox.checked").length==0){
				//如果不是必答，并且没有答
				isanswered = false;
			}else{
				//拼答案
				var $checkbox = $thisquestion.find(".content .option .ui.checkbox.checked");
				var optionId = $checkbox.parent().data("id");
				var optionVal = $checkbox.parent().data("val");
				var input_content ="";
				if($checkbox.siblings("input").length>0){
					input_content = $checkbox.siblings("input").val();
				}
			...
			if(!required && $thisquestion.find(".content .option .ui.checkbox.checked").length==0){
				...
				var length = $thisquestion.find(".content .option .ui.checkbox.checked").length
				if(length>max || length<min){
					$(".error h4").html("题目"+questionno+"不得少于"+min+"项，不得大于"+max+"项");
					return false;
				}
				//拼答案
				var answers=[];
				$thisquestion.find(".content .option .ui.checkbox.checked").each(function(){
			...
			if(required){
				...
				if(tablehasanswered && tablenotanswered){
					$(".error h4").html("量表题"+questionno+"没有回答完整，请补全！");
					return false;
				}
				//如果一行都没答，则返回答案为空
				if(!tablehasanswered){
