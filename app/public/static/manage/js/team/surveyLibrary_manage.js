...
//	$(".librarylist-container").height(window_height-100);
	$(".content .top").css("width",$("#content .container .bottom").width());
//	$(".content .top").css("margin-left",-(420+(scrollWidth/2)));
//	设置头部添加题目导航栏，顶部悬浮
//	var scrollWidth = getScrollWidth();
//	var contentWidth = $(".content-team").width()-scrollWidth;
//	var containerWidth = $(".content .container").width();
//	var libraryWidth = $(".librarylist-container").width();
//	var leftWidth1 = $(".ui.grid .ui.vertical.menu").outerWidth();
//	var leftWidth2 = (contentWidth/2)-476;
//	if(containerWidth>(contentWidth-libraryWidth)){
...
function initData(){
	...
	var successFunc = function(data){
		...
		for(var i in questionList){
			...
			$("#librarylist_tbody").append("<tr><td>"+title+"</td><td>"+typename+"</td></tr>")
		}
		...
		$("#libraryList_filter").parent().removeClass("right");
		$("#libraryList_filter").parent().removeClass("aligned");
		$("#libraryList_filter").parent().removeClass("eight");
		$("#libraryList_filter").parent().removeClass("wide");
		$("#libraryList_filter").parent().removeClass("column");
		$("#libraryList_filter").parent().addClass("left");
		$("#libraryList_filter").parent().addClass("aligned");
		$("#libraryList_filter").parent().addClass("sixteen");
		$("#libraryList_filter").parent().addClass("wide");
		$("#libraryList_filter").parent().addClass("column");
		$("#libraryList_wrapper>.ui.grid>.row:last").remove();
	}
	...
	if(!index){
		...
		getComponent("/static/manage/components/optionIMG.html",
				function(resultHTML){
					...
					image_index++;
				},{"-indexClass-":"uploadimg_"+image_index})
...
//新增题目
$(".content").on("click",".button-group-add .button",function(event){
...
//		      '<td><input placeholder="输入选项" type="text" name="option" /></td>'+
//		      '<td>'+
...
//			  '  </div>'+
//			  '</td>'+
//			  '<td><input placeholder="赋值" type="text" name="optionnum" value="'+optionNum+'"/></td>'+
//		      '<td>'+
...
//	$target = $(ele).parent().parent();
//	$option = $('<tr class="tr-option">'+
//			'<td><input placeholder="选项文字" type="text" name="option" /></td>'+
//			'<td><input placeholder="分数" type="text" name="score" /></td>'+
...
//	$target = $(ele).parent().parent();
//	$option = $('<tr class="tr-option">'+
//		'	<td><input placeholder="左侧文字" type="text" name="left-title" /></td>'+
//		'	<td><div class="ui checkbox" id="allow_right_title">'+
//		'		<input type="checkbox" name="allow-right-title" class="hidden">'+
//		'		<label></label>'+
//		'	</div><input placeholder="右侧文字" type="text" name="right-title" style="width:80%;" readonly/></td>'+
//		'	<td><input placeholder="最小" type="text" name="min" /></td>'+
//		'	<td><input placeholder="最大" type="text" name="max" /></td>'+
//		'	<td><input placeholder="步长" type="text" name="step" /></td>'+
...
//	if(!title || $.trim(title)==""){
//		if($root.hasClass("dl")){
//			$root.find(".error h4").html("段落内容不能为空！");
//		}else{
//			$root.find(".error h4").html("题目标题不能为空！");
//		}
//		return false;
//	}
...
//	if(max && min){
//		title = title+"(最多选"+max+"项，最少选"+min+"项)";
//	}
////	set标题
...
//		if($.trim(min)==""){
//			$root.find(".error h4").html("最小选项数不能为空！");
//			return false;
//		}else if(!parseInt(min)){
//			$root.find(".error h4").html("最小选项数格式不对");
//			return false;
//		}else if(parseInt(min)<=0){
//			$root.find(".error h4").html("最小选项数必须大于0");
//			return false;
//		}
//		if($.trim(max)==""){
//			$root.find(".error h4").html("最大选项数不能为空！");
//			return false;
//		}else if(!parseInt(max)){
//			$root.find(".error h4").html("最大选项数格式不对");
//			return false;
//		}else if(parseInt(max)<=0){
//			$root.find(".error h4").html("最大选项数必须大于0");
//			return false;
//		}
//		if(min>max){
//			$root.find(".error h4").html("最小选项数不能大于最大数！");
//			return false;
//		}
...
//					$root.find(".error h4").html("选项"+(i+1)+"的内容不能为空！");
//					return false;
//				}else if( allowInput==0 &&( !optionNum || $.trim(optionNum)=="")){
//					$root.find(".error h4").html("选项"+(i+1)+"的选项赋值不能为空！");
//					return false;
//				}
...
//					//放置一个填空选项
//					var $newoption = $('<div class="inline field">'+
//							'<div class="ui checkbox option allowinput"  data-optionnum="'+optionNum+'">'+
...
//					$root.find(".error h4").html("请给选项"+(i+1)+"的图片添加文字描述！");
//					return false;
//				}
//				if(!img || $.trim(img)==""){
//					$root.find(".error h4").html("选项"+(i+1)+"的图片不能为空！");
//					return false;
//				}else{
//					var $newoption = $('<div class="inline field">'+
//					'<div class="ui checkbox option option-img"  data-optionnum="'+optionNum+'">'+
//					'	<input type="checkbox" name="option" class="hidden">'+
...
//		if($.trim(leftTitle)==""){$root.find(".error h4").html("左侧标题不能为空");return false;}
...
//			if($.trim(rightTitle)==""){$root.find(".error h4").html("右侧标题不能为空");return false;}
...
//				$root.find(".error h4").html("左侧标题和右侧标题数量不一致");
//				return false;
//			}
//		}
...
//			if($.trim(optionName)==""){$root.find(".error h4").html("选项文字不能为空");return false;}
//			var optionScore = $(troption).find("input[name=score]").val();
//			if($.trim(optionScore)==""){$root.find(".error h4").html("分数不能为空");return false;}
//			else if(!parseInt(optionScore) && parseInt(optionScore)!=0){
//				$root.find(".error h4").html("分数格式不对");
//				return false;
//			}else if(parseInt(optionScore)<0){
//				$root.find(".error h4").html("分数不能小于0");
//				return false;
//			}
//			//添加第一行标题
...
//			if($.trim(maxlength)==""){
//				$root.find(".error h4").html("最大不能为空");return false;
//			}else if(!parseInt(maxlength) && parseInt(maxlength)!=0){
//				$root.find(".error h4").html("最大值格式不对");return false;
//			}
////			else if(parseInt(minlength)<=0){
...
//				$root.find(".error h4").html("最大值必须大于等于最小值");return false;
//			}
//			var step = $(troption).find("input[name='step']").val();
//			if($.trim(step)==""){
//				$root.find(".error h4").html("步长不能为空");return false;
//			}else if(!parseInt(step)){
//				$root.find(".error h4").html("步长格式不对");return false;
//			}else if(parseInt(step)<=0){
//				$root.find(".error h4").html("步长必须大于0");
//				return false;
//			}else if(parseInt(step)>(maxlength-minlength)){
//				$root.find(".error h4").html("步长数值过大");
//				return false;
//			}
//			step = parseInt(step);
//			//赋值
...
//	判断是否每个题目都已经完成编辑了
	...
	var url="/questionBank/addQuestionBank";
	var sendNum = 0;
//	题目
	...
	for(var i=0;i<questions.length;i++){
		...
		var successFunc = function(data){
			sendNum--;
			if(sendNum==0 && i==questions.length)
			alertokMsg(getLangStr("surveyLibrary_alert1"),getLangStr("alert_ok"),"window.location.href='/redirect?url=administrator/surveyLibrary_manage.jsp'");
		}
		sentJson(url,json,successFunc);
		sendNum++;
	}
...
function getJSON($this){
	...
	//向questionList中添加滑块题
		...
		setting = {"required":required,"items":items}
	}
	//把所有input的val放到attr val中
	$this.find("input").each(function(){
		$(this).attr("value",$(this).val());
	})
	//把所有textarea的text放到attr text中
	$this.find("textarea").each(function(){
		$(this).attr("value",$(this).val());
	})
	var html = $this.prop("outerHTML").replace(/\"/g,"'");
	return {
