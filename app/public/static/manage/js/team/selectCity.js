...
//                       { name: "辽宁", cities: [ "沈阳","大连","鞍山", "抚顺", "营口", "锦州", "丹东", "朝阳", "辽阳", "阜新", "铁岭", "盘锦", "本溪", "葫芦岛"] },  
...
var provinceAndCityArray =
    ...
    for(var el in provinceAndCityArray) {  
        ...
        provincesOptionHtml += "<option value=" + el + " data-log='"+provinceAndCityArray[el].log+"' data-lat='"+provinceAndCityArray[el].lat+"'>" + provinceAndCityArray[el].name + "</option>";  
    }  
    ...
    $("#selectProvince").bind("change", function() {  
        ...
        if(selectedProvince != -1) {  
            ...
            var cities = [];  
            cities = provinceAndCityArray[selectedProvince].children;  
            for(var elCity in cities) {  
                citiesOptionHtml += "<option value=" + elCity + " data-log='"+cities[elCity].log+"' data-lat='"+cities[elCity].lat+"'>" + cities[elCity].name + "</option>";  
            }  
            // 清空之前的城市列表  
...
//    根据名字查找省市，添加值
    function setDefaultValue(name){
    	for(var el in provinceAndCityArray) {  
            cities = provinceAndCityArray[el].children; 
            for(var i in cities) {  
                cityname = cities[i].name;
                if(cityname == name){
                	var citiesOptionHtml = "";  
                	for(var elCity in cities) {  
                		if(cities[elCity].name == name){
                			citiesOptionHtml += "<option value=" + elCity + " data-log='"+cities[elCity].log+"' data-lat='"+cities[elCity].lat+"' selected='true'>" + cities[elCity].name + "</option>";
                		}else{
                			citiesOptionHtml += "<option value=" + elCity + " data-log='"+cities[elCity].log+"' data-lat='"+cities[elCity].lat+"'>" + cities[elCity].name + "</option>";
                		}
                    } 
                    ...
                    $("#selectCity option[value!=-1]").remove();  
                    $("#selectCity").append(citiesOptionHtml);  
                	$("#selectProvince option[value="+el+"]").attr("selected",true);
                	return;
                }
    ...
    function getProvinceName(name){
    	for(var el in provinceAndCityArray) {  
            ...
            for(var i in cities) {  
                cityname = cities[i].name;
                if(cityname == name){
                	return provinceAndCityArray[el].name;
                }
            }  
        } 
    }