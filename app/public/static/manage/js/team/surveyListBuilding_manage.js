	...
	var successFunc = function(data){
		...
		for(i in surveys){
			...
			var htmlStr =$("<tr> <td><div class='ui fitted checkbox' data-id='"+id+"' data-count='"+count+"' ><input type='checkbox' class='hidden'><label></label></div></td><td>"+title+"</td> <td>"+introduction+"</td> <td>"+name+"</td> <td>"+count+"</td>" +
					"<td style='position:relative'><a class='answerUrl' data-url='http://"+host+"/views/mobile/mobileSurvey.jsp?surveyId="+id+"&teamId="+teamID+"&buildingId="+buildingID+"'>"+getLangStr("survey_copyurl")+"</a></td>" +
					"<td><a class='qrcode' style='text-decoration:underline' href='javascript:void(0)' data-url='"+host+"/views/mobile/mobileSurvey.jsp?surveyId="+id+"&teamId="+teamID+"&buildingId="+buildingID+"' >"+getLangStr("viewQR")+"</a></td></tr>") 
