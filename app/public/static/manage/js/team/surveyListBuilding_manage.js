...
;
//所有绑定过当前项目的问卷列表
var buildingSurveyList = [];
//获取问卷列表信息
function getSurveyData(){
	var url="/admin/project/single/building/survey";
//	var json={"projectID":teamID};
	...
	var successFunc = function(data){
		...
		for(i in surveys){
			...
			buildingSurveyList.push(survey.id)
			var id = survey.id;
			...
			var htmlStr =$("<tr> <td><div class='ui fitted checkbox' data-id='"+id+"' data-count='"+count+"' ><input type='checkbox' class='hidden'><label></label></div></td><td>"+title+"</td> <td>"+introduction+"</td> <td>"+name+"</td> <td>"+count+"</td>" +
					"<td style='position:relative'><a class='answerUrl' data-url='http://"+host+"/survey/mobileSurvey?role=admin&surveyId="+id+"&teamId="+teamID+"&buildingId="+buildingID+"'>"+getLangStr("survey_copyurl")+"</a></td>" +
					"<td><a class='qrcode' style='text-decoration:underline' href='javascript:void(0)' data-url='"+host+"/survey/mobileSurvey?role=admin&surveyId="+id+"&teamId="+teamID+"&buildingId="+buildingID+"' >"+getLangStr("viewQR")+"</a></td></tr>") 
...
$("#addSurvey").click(function(){
	window.location.href="/redirect?url=administrator/surveyAdd.jsp"
})
var surveyArr=[];
$("#bindSurvey").click(function(){
	//获取没有绑定过项目的问卷列表
	var url="/admin/project/single/building/survey_add";
	var json={"buildingID":buildingID};
	var successFunc = function(data){
		var list = data.list;
		$("#child").empty();
		for(var i=0;i<list.length;i++){
			...
			if($.inArray(notBindId, buildingSurveyList)!=-1){
				continue;
			}
...
$("#confirmBtn").click(function(){
	...
	$("#child .ui.checkbox.checked").each(function(){
		...
		var url = "/admin/project/single/building/survey_bind";
...
$("#surveyReport").click(function(){
	...
	$.cookie("gxendtime","");
	$.cookie("gxrelation",2);
	$.cookie("gxobjectId",buildingID);
	$.cookie("gxteamId",teamID);
	$.cookie("gxteamName",teamName);
	$.cookie("gxbuildingId",buildingID);
	$.cookie("gxbuildingName",buildingName);
	$.cookie("gxpointId","");
	$.cookie("gxpointName","");
	//window.location.href="/redirect?url=administrator/surveyReportBuilding.jsp?relation=2"
...
$("#surveyAnalysis").click(function(){
	...
	//window.location.href="/redirect?url=administrator/surveyAnalysisBuilding.jsp?relation=2&objectID="+buildingID;
...
$("#downloadqr").click(function(){
	...
	saveCanvas($canvas,'buildingQR','png');
});
//============================下载excel解析数据============================
		...
		for(var j in questionList){
			...
			if(type==1){//单选
				...
				for(var n in y_axis){
					...
					if(n==0){
						titleArr["question"+questionID+"-"+y_id]=questionOrder+questionOrder+getLangStr("surveyListDevice_export1")+y.left+"("+title+")";
					}else{
						titleArr["question"+questionID+"-"+y_id]=questionOrder+getLangStr("surveyListDevice_export2")+y.left+"("+title+")";
					}
				}
			}else if(type==4){//滑条
				...
				for(var n in items){
					...
					if(n==0){
						titleArr["question"+questionID+"-"+item_id]=questionOrder+questionOrder+getLangStr("surveyListDevice_export3")+item.left+"("+title+")";
					}else{
						titleArr["question"+questionID+"-"+item_id]=questionOrder+getLangStr("surveyListDevice_export4")+item.left+"("+title+")";
					}
				}
			}else if(type==0){//填空
				dataArr["question"+questionID]=[];
