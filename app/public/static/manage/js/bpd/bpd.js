...
$(function(){
	...
	$(".iCheck-helper").on("click",function(e){
		...
		if($col_len==0||$col_len==undefined){
			$("#" + $this_modal).find("button.close").removeClass("filter_100").addClass("filter_50");
		}else{
			$("#" + $this_modal).find("button.close").removeClass("filter_50").addClass("filter_100");
		}
...
function updataDatas($this_modal){
		...
		updateCharts(globleData,globleNumber);
}
...
function updateCharts(globleData,globleNumber){
	...
	addLoading(".chart-item>.bottom",1);
	$.ajax({
		url:"/building/gbd/statistical",
		type:"POST",
		data:{conditions:globleData_send,number:globleNumber},
		success:function(data){
			$("#num>span").html(data.data.count[0]);
			console.log("返回直方图数据成功————————————");
			console.log(data);
			if(data.code==200){
				showChart("Histogram",data);
				removeLoading();
			}
		},
	...
	$.ajax({
		...
		type:"POST",
		data:{conditions:globleData_send},
		success:function(data){
			console.log("返回散点图数据成功————————————");
			console.log(data);
			if(data.code==200){
				showChart("Scatter",data);
				removeLoading();
			}
		},
		error:function(error){
