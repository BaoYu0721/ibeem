    	   ...
    	   function swt_chart(data){
			   ...
			   $("#wst_chart_box").append(
			   		"<div class='ui bottom attached tab segment' data-tab='wst_"+ fenlei +"'>" +
			   			"<div id='wst_show_"+ fenlei +"' class='svgbox' style='width:57%'></div>"+
				"</div>"
			   	);
...
//    		   });
    	   ...
    	   $("#update_wstchart").click(function(){
        		   ...
        		   //$("#content").animate({scrollTop:$("#wst_chart").offset().top - 65},600);
...
/*     			dbtTimeResult = [];
     			...
     			for(var k=0;k<dbtTimeNum;k++){ // 这里不应该是dbtTimeNum 而是数组data的长度
     				var s = k + 1;
     				dbtTimeResult.push("节点" + s);
     			}*/
     			...
     			// 整理数据+画图
        		   ...
        		   //$("#content").animate({scrollTop:$("#dbtcharts_box").offset().top - 65},600);
	   		    }
    	   });
    	   ...
    	   function duibiCheck(deviceData,dataNum){
 			 	...
 			 	for(var j=1;j<dbtTimeArray.length;j++){
				  	...
				  	for(var i=0;i<data.length;i++){
						 if(data[i].timestemp > Number(startTimeStamp3 + dbtTimeArray[j-1]) && data[i].timestemp < Number(startTimeStamp3 + dbtTimeArray[j])){
							 dbtTimeDate_tem.push(data[i].tem);
							 dbtTimeDate_hum.push(data[i].hum);
							 dbtTimeDate_co2.push(data[i].co2);
							 dbtTimeDate_pm25.push(data[i].pm);
							 dbtTimeDate_sun.push(data[i].lightIntensity);
						 }
					 }
				  	
				  	dbtTimeObj_tem.name = getLangStr("devicedata_result3")+ j + getLangStr("devicedata_result4");
				  	dbtTimeObj_tem.data = dbtTimeDate_tem;
				  	temperature_duibi.push(dbtTimeObj_tem);
				  	
				  	dbtTimeObj_hum.name = getLangStr("devicedata_result3")+ j + getLangStr("devicedata_result4");
				  	dbtTimeObj_hum.data = dbtTimeDate_hum;
				  	humidity_duibi.push(dbtTimeObj_hum);
				  	
				  	dbtTimeObj_pm25.name = getLangStr("devicedata_result3")+ j + getLangStr("devicedata_result4");
				  	dbtTimeObj_pm25.data = dbtTimeDate_pm25;
				  	pm25_duibi.push(dbtTimeObj_pm25);
				  	
				  	dbtTimeObj_co2.name = getLangStr("devicedata_result3")+ j + getLangStr("devicedata_result4");
				  	dbtTimeObj_co2.data = dbtTimeDate_co2;
				  	co2_duibi.push(dbtTimeObj_co2);
				  	
				  	dbtTimeObj_sun.name = getLangStr("devicedata_result3")+ j + getLangStr("devicedata_result4");
 			 	...
 			 	dbt_chart("temperature",dataNum,temperature_duibi);
 			 	dbt_chart("hum",dataNum,humidity_duibi);
 			 	dbt_chart("sunshine",dataNum,sunshine_duibi);
 			 	dbt_chart("co2",dataNum,co2_duibi);
 			 	dbt_chart("pm25",dataNum,pm25_duibi);

    	   }
    	   
    	   
    	   // 对比图
    	   function dbt_chart(tit,dataNum,data){
    		   ...
    		   if(tit == "temperature"){
		        	...
		        	yAxisTitle = "(lux)";
		        	dbtChartTitle = getLangStr("devicedata_sun");
		        	dbtChartName = "sun";
		        }else if(tit == "co2"){
		        	yAxisTitle = "(ppm)";
		        	dbtChartTitle = getLangStr("devicedata_co2");
		        	dbtChartName = "co2";
		        }else if(tit == "pm25"){
		        	yAxisTitle = "(ug/m<sup>3</sup>)";
		        	dbtChartTitle = getLangStr("devicedata_pm25");
		        	dbtChartName = "pm25";
		        }
 				...
 				if(downDeviceId.length!=0){
					...
					var $step = $("#step").val(); 
 	    			...
 	    			$.ajax({
 	    	 			   type:"post",
 	    	 			   dataType:"json",
 	    	 			   url:"/device/download/create_work_order",
 	    	 			   data:{
	    ...
	    getComponent("/common/leftpanel",
		  		function(result){
		  			...
		  			$(".leftmenu").eq(2).addClass("active");
		  	});
    	
    	var teamName = $.cookie("teamname");
    	$(".teamTitleTit").text(teamName);
