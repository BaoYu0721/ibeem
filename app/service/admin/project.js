...
class ProjectService extends Service {
    ...
    async singleBuildingExport(buildingId){
        var result = null;
        try {
    ...
    async singleDeviceSearch(){
        var device = null;
        try {
            device = await this.app.mysql.query('select id, name from device where project_id is null');
        } catch (error) {
            return -1;
        }
        const deviceList = [];
    ...
    async singleDeviceAdd(projectId, ids){
        const { app } = this;
        const redlock = this.service.utils.lock.lockInit();
